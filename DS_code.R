...
slacktime<- 10 #add slack up and down
ped_time<- 10
...
if(nrow(DS_green_new[(abs(DS_green_new$diff_green)<slacktime)&(abs(DS_green_new$diff_green)<ped_time),])==0){
  print("calculated green time lies outside constraint")
  update<- FALSE
}else{
  update<- TRUE
  possible_times<- DS_green_new[(abs(DS_green_new$diff_green)<slacktime)&(abs(DS_green_new$diff_green)<ped_time),]
