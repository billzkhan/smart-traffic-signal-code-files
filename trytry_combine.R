...
# #for optimization 
# 
# #try to reduce the number of combhinations effort
bilal<- com
bilal$diff12<- bilal$Var1- bilal$Var2
bilal$diff13<- bilal$Var1- bilal$Var3
bilal$diff14<- bilal$Var1- bilal$Var4
bilal2<- bilal[!duplicated(bilal[,5:7]),]
bilal3<- bilal2[,c(1:4)]
bilal3<- bilal3[c(1:686698),]

BW_slack<- function(com){
  ...
  for (i in 1:nrow(com)){
    ...
    if(any(is.negative(constraint_w_up), na.rm=FALSE)){
      Negative2 <- max(which(constraint_w_up < 0))
...
output<- BW_slack(com = bilal3)

BW_AOG<- function(output,aog_e, aog_w){
...
output2<- BW_AOG(output, aog_e,aog_w)
...
write.csv(output2, "D:/output2_pehla.csv")
...
#extra
file1<- read.csv("D:/output2_1stpart.csv")
file2<- read.csv("D:/output2_2ndpart.csv")
T_file<- rbind(file1,file2)
# write.csv(T_file, "D:/actual_O.csv")

T_file<- T_file[,-c(7:18)]
T_file$diff12<- T_file$com1- T_file$com2
T_file$diff13<- T_file$com1- T_file$com3
T_file$diff14<- T_file$com1- T_file$com4
colnames(bilal)<- c("com1","com2","com3","com4","diff12","diff13","diff14")


T_file2<- merge(bilal,T_file, by = c("com1","com2","com3","com4","diff12","diff13","diff14"), all.x = TRUE)
newnew<- T_file2[complete.cases(T_file2),]
